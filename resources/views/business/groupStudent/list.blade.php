   ...
   <div class="block-header">
      <div class="row">
         ...
         <div class="col-lg-5 col-md-6 col-sm-12">
            <form action="{{ url('business/groupStudent/create') }}" method="get">
               ...
               <div class="body">
                  <div class="table-responsive">
                     <table class="table table-hover m-b-0">
                        ...
                        <tbody>
                           <tr  v-for="(GroupStudent,index) in GroupStudents.groupStudents">
                              <td>@{{GroupStudent.num}}</td>
                              <td>
                                 <p>@{{GroupStudent.period}}</p>
                              </td>
                              <td>
                                 <p>@{{GroupStudent.level}}</p>
                              </td>
                              <td>
                                 <p>@{{GroupStudent.section}}</p>
                              </td>
                              <td>
                                 <a :href="'{{url('/')}}/business/groupStudent/update/'+GroupStudent.id" class="btn btn-info"><i class="zmdi zmdi-edit"></i></a>
                                 <button class="btn btn-secondary"  @click="destroy(GroupStudent.id)">
   ...
   const business = new Vue({
       ...
       methods:{
         ...
         async destroy(id){
            ...
            Swal.fire({title: 'Estas seguro?',text: "No podrás revertir esto!",icon: 'warning',showCancelButton: true,confirmButtonColor: '#3085d6',cancelButtonColor: '#d33',confirmButtonText: 'Si, bórralo!',cancelButtonText: 'Cancelar'}).then((result) => {
               ...
               if (result.isConfirmed) {
                  ...
                  axios.post('{{ url("deleteGroupStudents") }}', {
         ...
         changePage(page){
            ...
            this.getGroupStudents(page);
   
         },//changePage()
            
         async getGroupStudents(page){
   
            let self = this;
   
            self.loading = true;
   
                  axios.post('{{ url("getGroupStudents") }}', {
                     ...
                     if(response.data.success==true){
                        ...
                        self.GroupStudents=response.data.GroupStudents;
   
                        self.paginate=response.data.GroupStudents.paginate;
