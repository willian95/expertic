                     ...
                     <div class="row justify-content-center">
                        <div class="col-12">
                           <div class="form-group">
                              ...
                              <input type="text" v-model="level" class="form-control" id="level" v-bind:class="{ 'is-invalid': errors.hasOwnProperty('level') }">
                              <small v-if="errors.hasOwnProperty('level')" class="text-danger ml-2">@{{ errors['level'][0] }}</small>
                           </div>
                        </div>
                     </div>
                  </div>
                     ...
                     <table class="table table-hover m-b-0">
                        <thead>
                           <tr>
                              ...
                              <th class="adjust-tr-80">Nivel</th>
                              <th class="adjust-tr-10">Acciones</th>
                           </tr>
                        </thead>
                        <tbody>
                           <tr  v-for="(levelItem,index) in    Levels.levels">
                              <td>@{{levelItem.num}}</td>
                              <td>
                                 <p>@{{levelItem.level}}</p>
                              </td>
                              <td>
                                 <button class="btn btn-info" @click="captureRecord(levelItem)">
                                 <i class="zmdi zmdi-edit"></i>
                                 </button>
                                 <button class="btn btn-secondary"  @click="destroy(levelItem.id)">
...
<script>
   const business = new Vue({
       ...
       methods:{
         ...
         async getLevels(page){
   
            let self = this;
   
            self.loading = true;
   
                  axios.post('{{ url("getLevels") }}', {
                     ...
                     if(response.data.success==true){
                        ...
                        self.paginate=response.data.Levels.paginate;
